...
// Si Colision
            if (tetromino.collides(i => ({ x: tetromino.x[i], y: tetromino.y[i] + 1 }))) {
                tetromino.merge();
// Lancement pour un nouveau tétromino
                tetromino = null;
...
// Vérifier les lignes complétées
                ...
                for (let y = FIELD_HEIGHT - 1; y >= MIN_VALID_ROW; --y)
                    if (FIELD[y].every(e => e !== false)) {
                        ...
                        ++completedRows;
            // Gardez la même ligne
                        ++y;
                    }

                if (completedRows) {
             // Relancer de nouveau le tableau
                    ctx.clearRect(0, 0, canvas.width, canvas.height);
...
// Si Pas de tetromino défaillant
        else {

            scoreLbl.innerText = score;
            linesLbl.innerText = lines;

// Créer un tétromino aléatoire
            tetromino = (({ x, y }, color) =>
